...
$active="clientes";
$sql_mod="SELECT * FROM clientes WHERE id='".escapa($_GET['id'])."'";
$result_mod=mysqli_query($link,$sql_mod);
$cuanto_mod=mysqli_num_rows($result_mod);
if ($cuanto_mod==0)
    {
        mysqli_free_result($result_mod);
...
if (isset($_POST['sb_mod_url']))
	{
		$nombre = escapa($_POST['nombre']);

        $sql_ins="UPDATE  clientes SET
            nombre='$nombre'
            WHERE id='$row_mod[id]'";

        mysqli_query($link,$sql_ins);

        header ("location:/admin/modificar_cliente.php?id=$row_mod[id]");
        exit;
...
if (isset($_POST['sb_ins_ur']))
	{
		...
		$cadena=$row_mod['dominios_permitidos'].",".$dominios;
		$cadena = ltrim($cadena, ',');
		


		$sql_ins="UPDATE clientes SET dominios_permitidos='$cadena' WHERE id='$row_mod[id]'";
		mysqli_query($link,$sql_ins);
				...
				<div class="row">
					<div class="col-md-5">
						<div class="form-group">
							...
							<input type="text" name="nombre" class="form-control" id="nombre" value="<?php echo $row_mod['nombre'];?>" placeholder="Nombre" reqruired>
						</div>						
					</div>
					...
					<div class="col-sm-2">
						<label>&nbsp;</label>
						<input type="submit" name="sb_mod_url" class="btn btn-success btn-block" value="Modificar" />
					</div>				
				</div>
                </form>
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body table-responsive p-0">
                            ...
                            <tbody>
                                <?php
                                $elementos = explode(',', $row_mod['dominios_permitidos']);
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center" style="width:50px;">
                                            <a href="javascript:void(0)" class="delete-domain" 
                                                data-index="<?php echo $index; ?>" 
                                                data-domain="<?php echo trim($nombre); ?>" 
                                                data-id="<?php echo $row_mod['id']; ?>"> <!-- Enviar el ID del cliente -->
                        ...
                        <div class="card-footer text-right pt-4 pb-4">
                            <a href="/admin/cliente.php" class="btn btn-success">Volver</a>
                        </div>				
                        </form>					
                    </div>
                </div>
